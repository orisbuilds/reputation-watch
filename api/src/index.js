/**
 * Reputation Watch — API (Cloudflare Worker)
 * -------------------------------------------------------------
 * A tiny, free-tier backend for the Reputation Watch prototype.
 *
  ...
  const html = `
    <div style="font-family:system-ui,Segoe UI,Arial,sans-serif;max-width:560px;margin:0 auto;color:#10201e">
      <div style="background:#0f766e;color:#fff;padding:18px 22px;border-radius:12px 12px 0 0">
        <div style="font-size:12px;letter-spacing:1px;text-transform:uppercase;opacity:.85">Reputation Watch</div>
      ...
      <p style="color:#9aa5a3;font-size:11px;text-align:center;margin:14px 0 0">Reputation Watch · sent because a patient used your feedback QR code</p>
    </div>`;
...
export default {
  async fetch(request, env) {
    ...
    try {
      // ---- health ----
      if (path === "/api/health") return json({ ok: true, service: "reputation-watch" });
